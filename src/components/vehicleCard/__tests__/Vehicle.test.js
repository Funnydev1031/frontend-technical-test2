...
describe('<Vehicle /> Tests', () => {
  it('should render vehicle name', () => {
    const { queryByTestId } = render(<VehicleCard vehicle={mockVehicle} />);
  ...
  it('should render vehicle price', () => {
    ...
    expect(queryByTestId('vehicle-price').innerHTML).toBe(
      `From ${mockVehicle.price}`
    );
  });
  ...
  it('should render vehicle description', () => {
    ...
    expect(queryByTestId('vehicle-description')).toBe(
  ...
  it('should show the vehicle image', () => {
    ...
    expect(
      queryByTestId('vehicle-image').querySelector('img').src
    ).toContain(mockVehicle.media[1].url);
  });

  it('should hide price if not set', () => {
    const { queryByTestId } = render(
      <VehicleCard vehicle={mockVehicleNoPrice} />
    );
